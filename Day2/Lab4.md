...
Log into the server and copy the TimeCalibration folder.
```sh
cp -r /home/ubuntu/Share/TimeCalibration .
#Move into the directory with cd and activate the conda environment
...
```
After running you'll have the Hessian Matrix (.hessian), and the control file to run the prior and the posterior calibration with MCMCTree (.ctl). It takes 11 hr to run, for this reason we have already generated the matrix for you. 
**Kill** the previous command with `CTRL+C`.
You will have 2 folders `Prior` and `Posterior`, each with the files needed for the estimates.

The control file generated by IQTREE is still under developement, therefore we refer to the previous version originally from the `Paml` package.
...
Well done! You are now an expert in node calibration. Let's move on the following section on ancestral state estimation.

# Ancestral state estimation

Ok we have our tree, we have estimated when molluscs have diverged, but we want to dig further. We want to know if the last common ancestor of molluscs had a shell, and if so which type of shell. To do so we are going to do an ancestral state estimation using the MK model. 
We have compiled a little character matrix that you can open in excel, called `shell_matrix.csv`. You will find it here in the `Data/Lab4/ASE` folder. Download the matrix and the tree called `Mollusca_LGC60_BL_rooted.tre`.
Have a look at it. For this excercise we have 2 characters:
